...
- `thread/start`, `thread/resume`, and `thread/fork` responses include the legacy `sandbox` compatibility projection. Those responses and `thread/started` notifications include `modelProviderUsesManifest`, which reports whether the thread carries manifest-scoped provider lineage—including after switching back to an ordinary provider—so clients can decide whether a thread-scoped model-list refresh is required; older servers may omit it. `instructionSources` lists loaded instruction files using each source environment's native absolute path syntax, including files loaded from remote environments. Experimental clients can read `runtimeWorkspaceRoots` for the thread-scoped runtime roots and `activePermissionProfile` for the named or implicit built-in profile identity/provenance when known. Their deprecated experimental `multiAgentMode` field, and the corresponding thread setting, always report `explicitRequestOnly`; Ultra reasoning effort is the source of proactive multi-agent behavior.
...
- `model/list` — list available models (set `includeHidden: true` to include entries with `hidden: true`; set `threadId` to list a loaded thread's effective provider catalog instead of the startup provider catalog). The response includes `modelProviderUsesManifest` so clients can tell whether manifest-scoped selection semantics apply, including inherited manifest lineage after a provider switch. Model entries include model-advertised string reasoning effort options in the catalog's intended progression order, `additionalSpeedTiers`, `serviceTiers`, optional `defaultServiceTier`, optional legacy `upgrade` model ids, optional `upgradeInfo` metadata (`model`, `upgradeCopy`, `modelLink`, `migrationMarkdown`), and optional `availabilityNux` metadata. `defaultReasoningEffort` remains the legacy non-null effective default for existing clients, while `advertisedDefaultReasoningEffort` preserves the exact source metadata. When `modelProviderUsesManifest` is true, clients should use the advertised field and omit an effort override when it is `null`. Clients should preserve the `supportedReasoningEfforts` array order rather than deriving order from the effort names.
